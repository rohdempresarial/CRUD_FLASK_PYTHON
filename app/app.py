...
def formAddCarro():
    if request.method == 'POST':
        ...
        if(request.files['foto'] !=''):
            ...
            resultData = registrarCarro(marca, modelo, year, color, cilindraje, velocidad, puertas, favorito, nuevoNombreFile)
            if(resultData ==1):
...
def  formActualizarCarro(idCarro):
    if request.method == 'POST':
        ...
        if(request.files['foto']):
            ...
            resultData = recibeActualizarCarro(marca, modelo, year, color, cilindraje, velocidad, puertas, favorito, fotoForm, idCarro)
        else:
            fotoCarro  ='sin_foto.jpg'
            resultData = recibeActualizarCarro(marca, modelo, year, color, cilindraje, velocidad, puertas, favorito, fotoCarro, idCarro)
