...
def registrarCarro(marca='', modelo='', year='', color='', cilindraje='', velocidad='', puertas='', favorito='', nuevoNombreFile=''):       
        conexion_MySQLdb = connectionBD()
        cursor           = conexion_MySQLdb.cursor(dictionary=True)
            
        sql         = ("INSERT INTO carros(marca, modelo, year, color, cilindraje, velocidad, puertas, favorito, foto) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)")
        valores     = (marca, modelo, year, color, cilindraje, velocidad, puertas, favorito, nuevoNombreFile)
...
def  recibeActualizarCarro(marca, modelo, year, color, cilindraje, velocidad, puertas, favorito, nuevoNombreFile, idCarro):
        conexion_MySQLdb = connectionBD()
        cur = conexion_MySQLdb.cursor(dictionary=True)
        cur.execute("""
            UPDATE carros
            SET 
                ...
                foto    = %s
            WHERE id=%s
            """, (marca,modelo, year, color, cilindraje, velocidad, puertas, favorito, nuevoNombreFile,  idCarro))
        conexion_MySQLdb.commit()
